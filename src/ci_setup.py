"""
CI/CD Setup Script
Generates realistic sample data and trains model
...
print(f"Generating {nc} customers and {n} transactions...")

# ── Generate features with realistic values ────────

num_transactions       = np.random.randint(1, 100, nc)
num_unique_invoices    = (
    num_transactions *
    np.random.uniform(0.5, 1.0, nc)
).astype(int).clip(1)
avg_transaction_amount = np.random.uniform(10, 500, nc)
max_transaction_amount = (
    avg_transaction_amount *
    np.random.uniform(1.5, 5, nc)
)
min_transaction_amount = (
    avg_transaction_amount *
    np.random.uniform(0.1, 0.8, nc)
)
std_transaction_amount = (
    avg_transaction_amount *
    np.random.uniform(0.1, 0.5, nc)
)
median_transaction     = (
    avg_transaction_amount *
    np.random.uniform(0.8, 1.2, nc)
)
...
num_unique_desc        = (
    num_unique_products +
    np.random.randint(0, 5, nc)
)
avg_items              = np.random.uniform(1, 20, nc)
high_value_ratio       = np.random.uniform(0, 1, nc)
m_score                = np.random.randint(1, 6, nc)

# ── Target variable depends on features ───────────
# This creates realistic correlation
# Model can actually learn from this

total_spend = (
    num_transactions * avg_transaction_amount * 0.9 +
    num_unique_invoices * avg_transaction_amount * 0.5 +
    m_score * 1000 +
    num_unique_products * 100 +
    high_value_ratio * 3000 +
    np.random.normal(0, 200, nc)  # less noise
).clip(10)
...
transactions = pd.DataFrame({
    ...
    "InvoiceDate" : pd.date_range(
        "2010-01-01", periods=n, freq="1min"
    ).astype(str),
...
transactions.to_csv(
    "data/clean_transactions.csv", index=False
)
customers.to_csv(
    "data/customer_features.csv", index=False
)
